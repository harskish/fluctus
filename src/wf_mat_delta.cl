...
#define BXDF_USE_IDEAL_REFLECTION
#define BXDF_USE_IDEAL_DIELECTRIC
#include "bxdf_partial.cl"

kernel void wavefrontDelta(
...
)
{
    ...
    if (gid_direct >= queueLens->deltaQueue)
        return;

    uint gid = deltaQueue[gid_direct];
